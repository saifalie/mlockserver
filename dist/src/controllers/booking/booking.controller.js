...
export const cancelBooking = async (req, res, next) => {
    ...
    try {
        ...
        await session.commitTransaction();
        res.status(StatusCodes.OK).json(new ApiResponse(StatusCodes.OK, { booking }, 'Booking cancelled successfully'));
    }
    catch (error) {
        await session.abortTransaction();
        next(error);
...
export const processExtraTimePayment = async (req, res, next) => {
    try {
...
export const verifyCheckoutPayment = async (req, res, next) => {
    ...
    try {
        ...
        const extraTimeSeconds = parseInt(razorpayOrder.notes?.extraTimeSeconds?.toString() || "0" // Add optional chaining for `notes`
        );
        if (!(RAZORPAY_KEY_ID && RAZORPAY_KEY_SECRET)) {
            throw new InternalException('Credentials not found', ErrorCode.SECRET_KEY_NOT_FOUND, { message: 'razopay credentials not found' });
        }
        ...
        }, { new: true, session }).populate('locker lockerStation');
        ...
        res.status(StatusCodes.OK).json(new ApiResponse(StatusCodes.OK, { booking, payment }, 'Payment verified and checkout completed'));
    }
    catch (error) {
        await session.abortTransaction();
        next(error);
...
export const directCheckout = async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
...
async function performCheckout(booking, session, checkoutTime) {
